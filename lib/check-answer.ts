...
/**
 * Grading a referee's written decision against the stored answer.
 *
 * Lifted out of /api/check-answer so the public guest route can grade without
 * the browser ever seeing scenarios.ai_answer. That route is open to anyone
 * with a pack link, so the answer key has to stay server-side: the guest sends
 * their words, the server sends back a verdict.
 */

export interface AnswerCheck {
  isCorrect: boolean
  confidence: number
  /** Short summary of the correct call. Empty when the scenario has no answer. */
  verdict: string
  /** Why the referee's answer was right or wrong. */
  explanation: string
}

/** The bar the app has always used for "correct": the model has to mean it. */
...
User Answer: ${userAnswer}`,
  ...
  try {
    ...
    return {
      isCorrect: false,
      confidence: 0,
      verdict: hasStoredAnswer ? fallbackVerdict : "",
      explanation: hasStoredAnswer ? fallbackDetail : "",
    }
  }
}
