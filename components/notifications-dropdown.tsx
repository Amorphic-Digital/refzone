...
export function NotificationsDropdown({ trigger, align = "end" }: NotificationsDropdownProps) {
  ...
  const loadNotifications = async () => {
    try {
      ...
      const relatedUserIds = [...new Set(
        (notificationsData || []).map((n: Notification) => n.related_user_id).filter(Boolean)
      )]
      ...
      if (relatedUserIds.length > 0) {
        ...
        profiles?.forEach((p: { id: string; display_name: string }) => profileMap.set(p.id, { display_name: p.display_name }))
      }

      const notificationsWithProfiles = (notificationsData || []).map((notification: Notification) => ({
        ...notification,
